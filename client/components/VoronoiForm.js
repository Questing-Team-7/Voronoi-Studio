...
export default () => {
	...
	const [voronoiArtData, setVoronoiArtData] = useState([]);
	...
	//filters through art data without requerying the met
	useEffect(() => {
		let workingArtData = [...artData];
		if (femaleArtist)
			workingArtData = workingArtData.filter((val) => {
				return val.artistGender === "Female";
			});
		setVoronoiArtData(workingArtData);
	}, [artData, femaleArtist]);

	//creates a new voronoi when we get new vornoi art data or cell counts
	useEffect(() => {
		try {
			//resets progress bar
			setCurrentProgress(0);
			...
			} else if (voronoiArtData.length === 1) {
